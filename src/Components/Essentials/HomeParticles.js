...
var wid = x - (x % 100);
var hig = y - (y % 100);
var canvas_width = wid;
var canvas_height = hig;
...
function init() {
  canvas = document.getElementById("liq");
...
function draw() {
  ...
  ctx.strokeStyle = "#00FFFF"; //This sets the color of the particles
  update_particle();
  for (i = 0; i < vec_cells.length; i++) {
    var cell_datas = vec_cells[i];
...
function update_velocity(cell_data) {
  ...
  cell_data.yv +=
    (cell_data.up_left.pressure * 0.5 +
      ...
      cell_data.down_right.pressure * 0.5) *
    0.25;
  cell_data.xv *= 0.75; //These two lines cause velocity degradation over time so the particles don't move infinitely
  cell_data.yv *= 0.75;
}
